...
/*ISR(USARTF0_TXC_vect) {
	if (_uart_buffer_f0.tx_buffer_start != _uart_buffer_f0.tx_buffer_end) {
		// There is actually data to send, so increment the start position and send that byte. Make sure to handle wrap arounds
		USARTF0.DATA = _uart_buffer_f0.tx_buffer[_uart_buffer_f0.tx_buffer_start];	// write the data into the output buffer
		_uart_buffer_f0.tx_buffer_start = (_uart_buffer_f0.tx_buffer_start+1)%_uart_buffer_f0.tx_buffer_size;
	}
}
...
ISR(USARTF0_RXC_vect) {
	...
	if (((_uart_buffer_f0.rx_buffer_start != 0) && (_uart_buffer_f0.rx_buffer_end != _uart_buffer_f0.rx_buffer_start-1)) 
	   || ((_uart_buffer_f0.rx_buffer_start == 0) && (_uart_buffer_f0.rx_buffer_end != _uart_buffer_f0.rx_buffer_size-1))) {
		// if there is room in the rx buffer, then put it in
		_uart_buffer_f0.rx_buffer[_uart_buffer_f0.rx_buffer_end] = USARTF0.DATA;
		_uart_buffer_f0.rx_buffer_end = ((_uart_buffer_f0.rx_buffer_end+1) % _uart_buffer_f0.rx_buffer_size);
	}
	else 
...
int uart_connect_port(uart_port_t *port) {
	...
	//port->uart_register->CTRLA = USART_TXCINTLVL_MED_gc;
	return 0;
}
...
int uart_tx_data(uart_port_t *port, uint8_t *data, uint8_t data_length) {
	...
	for (byte_cnt = 0; byte_cnt < data_length; byte_cnt++) {
		...
		// if the buffer is not full, then we can copy a byte into the buffer
		current_buffer->tx_buffer[current_buffer->tx_buffer_end] = data[byte_cnt];
...
int uart_rx_data(uart_port_t *port, uint8_t *data, uint8_t data_length) {
	...
	for (byte_cnt = 0; byte_cnt < data_length; byte_cnt++) {
		...
		data[byte_cnt] = current_buffer->rx_buffer[current_buffer->rx_buffer_start];
